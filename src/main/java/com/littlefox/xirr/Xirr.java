...
public class Xirr {
     ...
     */
    public Xirr(double guess, Transaction... tx) {
     ...
     * @throws IllegalArgumentException if all the transactions non-negative (withdrawals)
     */
    public Xirr(Collection<Transaction> txs, double guess) {
        this(txs, null, guess);
    }
    ...
    private Investment createInvestment(Transaction tx) {
        ...
        result.years = DAYS.between(tx.when, details.end) / tx.ty;
        return result;
    }
     ...
     * @return
     */
    public double xirr(double target) {
        final double years = DAYS.between(details.start, details.end) / details.days;
        if (details.maxAmount == 0) {
            return -1; // Total loss
        }
